...
### tables ###
    ...
    desc_tbl <- reactive({ dsc_tbl_forShiny %>% filter(`Site ID`==sID()) %>% gather("reference_db",'record') }) 
    
    output$DESC_TBL <-renderTable(desc_tbl())
        
    scz_asd_tbl <- reactive(scz_asd_long %>% filter(siteID==sID()) %>% select(-siteID))
    output$SCZ_ASD_TBL <- renderTable(left_join(scz_asd_regions, scz_asd_tbl()))
    
    
### histogram ###    
    
    hstPlot <- reactive({
        meanAP_distn_binned %>% ggplot(aes(x=meanAP)) + 
...
### site Ed per Cell ###
    ...
    site_by_gp <- reactive({ 
        ...
        sites_cells_slim %>% filter(siteID == sID()) %>% 
                            ...
                            # req(input$ed_siteID)
                            # waiter::Waiter$new(id = "SITE_BY_GPID")$show()                       
...
### sites FC over gene length ###   
    ...
    p <- reactive(
     ...
     output$PLOTLY_GENE <- renderPlotly({
         ...
         print(
                ggplotly( p() , tooltip=c("text"), source = "mySLCT" )) })
     
    output$PTLY_CLICK <- renderPrint({
        
        evnt_dat <- event_data('plotly_click', source="mySLCT")
        evnt_out <- ifelse(!is.na(evnt_dat),
        paste0(chrID(),'_', unique(evnt_dat[3])),
        "")
        
        new_inpt <- unique(unlist(evnt_out))[1]
        print(new_inpt)
        
    })
    
    
    observeEvent( event_data('plotly_click', source="mySLCT"),
                  {
         evnt_dat <- event_data('plotly_click', source="mySLCT")
         new_inpt <- unique(paste0(chrID(),'_', evnt_dat[3]))
         updateVarSelectInput(session, "ed_siteID", selected = new_inpt)
            })
